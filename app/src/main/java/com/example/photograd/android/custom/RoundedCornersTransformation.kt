...
class RoundedCornersTransformation @JvmOverloads constructor(
    ...
    private fun drawTopLeftRoundRect(canvas: Canvas, paint: Paint, right: Float, bottom: Float) {
        canvas.drawRoundRect(
            RectF(margin, margin, (margin + diameter), (margin + diameter)),
            radius,
            radius,
            paint
        )
        canvas.drawRect(
            RectF(margin, (margin + radius), (margin + radius), bottom),
            paint
        )
    ...
    private fun drawBottomLeftRoundRect(canvas: Canvas, paint: Paint, right: Float, bottom: Float) {
        ...
        canvas.drawRect(
            RectF(margin, margin, (margin + diameter), bottom - radius),
            paint
        )
        canvas.drawRect(RectF((margin + radius), margin, right, bottom), paint)
    }
    ...
    override fun hashCode(): Int {
        return ID.hashCode() + radius.toInt() * 10000 + diameter.toInt() * 1000 + margin.toInt() * 100 + cornerType.ordinal * 10
    }
